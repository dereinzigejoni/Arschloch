...
class HandSpec extends AnyFunSuite {
  test("empty hand has value 0") {assert(Hand.empty.value == 0)}
  ...
  test("multiple aces adjust correctly") {
    val h = Hand.empty.add(Card("A", Hearts)).add(Card("A", Diamonds)).add(Card("9", Clubs))
    assert(h.value == 21)
  }
  test("bust detection") {
    val h = Hand.empty.add(Card("10", Hearts)).add(Card("9", Clubs)).add(Card("5", Spades))
    assert(h.isBust)
  }
