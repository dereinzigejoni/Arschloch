// src/main/scala/de/htwg/blackjack/gui/BlackjackGuiApp.scala
...
object BlackjackGuiApp extends JFXApp3 with GameObserver {
  ...
  /** Karte als ImageView laden */
  private def loadCardImage(c: Card): ImageView = {
    // Wir bauen den Pfad exakt wie resources/cards/Clubs2.png
    val fileName = s"/img/cards/${c.suit.toString}${c.rank.symbol}.png"
    // Versuche, die Resource als URL zu laden
    val urlOpt = Option(getClass.getResource(fileName))
    val img = urlOpt match {
      case Some(url) =>
        // OK, wir haben sie gefunden
        new Image(url.toString)
      case None =>
        // NOT FOUND → Fehlermeldung in Konsole + Rückfall‐Bild
        System.err.println(s"[ERROR] Karte nicht gefunden: $fileName")
        new Image(getClass.getResource("/cards/back.png").toString)
    }
    // Baue das ImageView
    new ImageView(img) {
      fitWidth  = 80
